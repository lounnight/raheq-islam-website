...
export type MethodParams = {
    ...
    ishaAngle?: number;
    ishaMinutes?: number;
    ramadanIshaMinutes?: number;
};

export const CALCULATION_METHODS: Record<CalculationMethodKey, MethodParams> = {
...
export const METHOD_LABELS: Record<CalculationMethodKey, string> = {
    ...
    gulf_region: 'منطقة الخليج',
    kuwait: 'الكويت',
    qatar: 'قطر',
    singapore: 'سنغافورة (MUIS)',
    france: 'فرنسا (UOIF)',
    turkey: 'تركيا (Diyanet)',
    russia: 'روسيا',
    ...
    tunisia: 'تونس',
    algeria: 'الجزائر',
    indonesia: 'إندونيسيا (KEMENAG)',
    morocco: 'المغرب'
};

export const SUPPORTED_METHODS: string[] = Object.keys(CALCULATION_METHODS);

export const SUPPORTED_MADHABS: string[] = ['shafi', 'hanafi'];

export const MADHAB_LABELS: Record<string, string> = {
    shafi: 'الجمهور (الشافعي، المالكي، الحنبلي)',
    hanafi: 'الحنفي'
};

const MADHAB_ASR_FACTOR: Record<string, number> = { shafi: 1, hanafi: 2 };
...
const julianDate = ({ year, month, day }: { year: number; month: number; day: number }) => {
    let y = year;
    let m = month;
    if (m <= 2) {
        y -= 1;
...
const sunHourAngle = (altitude: number, latitude: number, declination: number) => {
    const cosH =
...
const solarNoon = (longitude: number, utcOffset: number, equationOfTime: number) =>
    12 * 60 + utcOffset * 60 - longitude * 4 - equationOfTime * 60;

const asrAltitude = (latitude: number, declination: number, factor: number) => {
    const tangent = Math.abs(tand(latitude - declination));
    return Math.atan(1 / (tangent + factor)) * (180 / Math.PI);
};

const highLatCorrection = (prayer: string, angle: number, sunrise: number, maghrib: number) => {
...
export type ComputePrayerTimesOptions = {
    year: number;
    month: number;
    day: number;
    latitude: number;
...
export type CalculatedTimesMinutes = {
    fajr: number | null;
    sunrise: number | null;
    dhuhr: number;
    asr: number | null;
...
export const computePrayerTimes = (options: ComputePrayerTimesOptions): CalculatedTimesMinutes => {
    const {
        year,
        month,
        day,
    ...
    const methodParams = CALCULATION_METHODS[method as CalculationMethodKey] || CALCULATION_METHODS.umm_al_qura;
    const asrFactor = MADHAB_ASR_FACTOR[madhab] || 1;
    ...
    const hourAngle = (altitude: number) => sunHourAngle(altitude, latitude, declination);
    const minutes = (h: number | null) => (h === null ? null : h * 4);
    ...
    if (typeof methodParams.ishaMinutes === 'number') {
        ...
        let t = minutes(hourAngle(methodParams.ishaAngle ? -methodParams.ishaAngle : -18));
...
const ummAlQuraHijriMonth = ({ year, month, day }: { year: number; month: number; day: number }) => {
    let m = month;
    let y = year;
    if (m < 3) {
        y -= 1;
...
const isRamadan = (date: { year: number; month: number; day: number }) => ummAlQuraHijriMonth(date) === 9;
const pad2 = (n: number) => (n < 10 ? `0${n}` : `${n}`);

export const formatMinutes = (minutes: number | null | undefined, hours12: boolean = false): string | null => {
